...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const loggedInUser = await login({ email, password }, "/auth/login");
      const redirectPath = location.state?.from?.pathname || getDashboardPathByRole(loggedInUser?.role);
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="user@nitp.ac.in" className="w-full pl-10 pr-4 py-2.5 bg-white border border-border rounded-[10px] text-base text-structural focus:border-accent focus:outline-none" required />
            </div>
          </div>
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between">
              ...
              <Link to="/forgot-password" className="text-xs font-semibold text-structural-muted hover:text-structural transition-colors">
                Forgot password?
              </Link>
            </div>
            <div className="relative">
              <Lock size={18} className="absolute left-3 top-3 text-structural-muted" />
              <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" className="w-full pl-10 pr-4 py-2.5 bg-white border border-border rounded-[10px] text-base text-structural focus:border-accent focus:outline-none" required />
            </div>
